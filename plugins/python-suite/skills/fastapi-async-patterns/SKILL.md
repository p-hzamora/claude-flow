...
async def io_intensive():
    async with httpx.AsyncDocument() as document:
        response = await document.get('https://api.example.com/data')
...
import asyncpg
from motor.motor_asyncio import AsyncIOMotorDocument
...
# MongoDB with Motor
mongo_document = AsyncIOMotorDocument('mongodb://localhost:27017')
db = mongo_document.mydatabase
...
@app.websocket('/ws/chat/{document_id}')
async def chat_endpoint(websocket: WebSocket, document_id: str):
    await manager.connect(websocket)
    await manager.broadcast(f'Document {{document_id}} joined the chat')
    try:
        while True:
            data = await websocket.receive_text()
            await manager.broadcast(f'Document {{document_id}}: {data}')
    except WebSocketDisconnect:
        manager.disconnect(websocket)
        await manager.broadcast(f'Document {{document_id}} left the chat')
...
One-way streaming from server to document.
...
async def aggregate_user_data(user_id: int):
    async with httpx.AsyncDocument() as document:
        ...
        profile_task = document.get(f'https://api.example.com/users/{user_id}')
        posts_task = document.get(f'https://api.example.com/users/{user_id}/posts')
        comments_task = document.get(f'https://api.example.com/users/{user_id}/comments')
...
async def get_fastest_price(product_id: str):
    async with httpx.AsyncDocument() as document:
        tasks = [
            document.get(f'https://store1.com/price/{product_id}'),
            document.get(f'https://store2.com/price/{product_id}'),
            document.get(f'https://store3.com/price/{product_id}')
        ]
...
async def lifespan(app: FastAPI):
    ...
    redis_document = await create_redis_document()

    # Store in app state
    app.state.db_pool = db_pool
    app.state.redis = redis_document
...
Efficient connection management for databases and HTTP documents.
...
# HTTP document pool
class HTTPDocumentPool:
    def __init__(self):
        self.document = None

    async def get_document(self) -> AsyncGenerator[httpx.AsyncDocument, None]:
        if self.document is None:
            self.document = httpx.AsyncDocument(
    ...
    async def close(self):
        if self.document:
            await self.document.aclose()

http_pool = HTTPDocumentPool()

@app.get('/external-api')
async def call_external_api(document: httpx.AsyncDocument = Depends(http_pool.get_document)):
    response = await document.get('https://api.example.com/data')
    return response.json()
```
...
- [HTTPX Async Document](https://www.python-httpx.org/async/)

...
### FastAPI Test Document

```python
# conftest.py
import pytest
from fastapi.testdocument import TestDocument
from app.main import app

@pytest.fixture
def document():
    """FastAPI test document."""
    return TestDocument(app)

# test_api.py
def test_read_root(document):
    response = document.get("/")
    assert response.status_code == 200
    assert response.json() == {"message": "Hello World"}

def test_read_item(document):
    response = document.get("/items/1")
    assert response.status_code == 200
    assert response.json() == {"item_id": 1, "name": "Item 1"}

def test_read_item_not_found(document):
    response = document.get("/items/0")
    assert response.status_code == 404
    assert response.json() == {"detail": "Item not found"}

def test_create_item(document):
    response = document.post(
        "/items",
...
# conftest.py
import pytest
from httpx import AsyncDocument
from app.main import app

@pytest.fixture
async def async_document():
    """Async test document for FastAPI."""
    async with AsyncDocument(app=app, base_url="http://test") as document:
        yield document
...
@pytest.mark.asyncio
async def test_read_root_async(async_document):
    response = await async_document.get("/")
    assert response.status_code == 200
    assert response.json() == {"message": "Hello World"}

@pytest.mark.asyncio
async def test_create_item_async(async_document):
    response = await async_document.post(
        "/items",
...
def document(test_db):
    ...
    with TestDocument(app) as test_document:
        yield test_document

    app.dependency_overrides.clear()

# test_users.py
def test_create_user(document):
    response = document.post(
        "/users",
...
def test_read_users(document):
    # Create user first
    document.post("/users", json={"email": "user1@example.com", "password": "pass1"})
    document.post("/users", json={"email": "user2@example.com", "password": "pass2"})

    # Read users
    response = document.get("/users")
    assert response.status_code == 200
...
# test_views.py
import pytest
from django.test import Document
from myapp.models import User
...
@pytest.mark.django_db
def test_user_list_view(document):
    # Create test data
...
# test_api.py
import pytest
from rest_framework.test import APIDocument
from myapp.models import User
...
@pytest.mark.django_db
def test_list_users(api_document):
    User.objects.create(email="user1@example.com", name="User 1")
    User.objects.create(email="user2@example.com", name="User 2")

    response = api_document.get("/api/users/")
    assert response.status_code == 200
    assert len(response.data) == 2

@pytest.mark.django_db
def test_create_user(api_document):
    data = {"email": "new@example.com", "name": "New User"}
    response = api_document.post("/api/users/", data)
...
async def fetch_data(url):
    async with aiohttp.DocumentSession() as session:
...
async def test_fetch_data(mocker):
    ...
    mocker.patch("aiohttp.DocumentSession", return_value=mock_session)

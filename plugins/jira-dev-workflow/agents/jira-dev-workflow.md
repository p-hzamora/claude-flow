...
- Tell it explicitly where to write specs: `.claude/planning/<TICKET-KEY>/specs/` (per-ticket subdirectory, not a flat file in `.claude/planning/`). If this path isn't in the handoff prompt, sdd-orchestrator defaults elsewhere. `.claude/` is gitignored, so these files never dirty the worktree or show up in `git status`/commits.

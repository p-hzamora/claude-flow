...
- **PDF converter script**: confirm `${CLAUDE_PLUGIN_ROOT}/scripts/md2pdf.sh` exists and is executable. This script — not a hand-rolled `pandoc`/`weasyprint`/`wkhtmltopdf`/`cupsfilter` invocation — is the only sanctioned way to turn Phase 6's Markdown summary into a PDF; it self-installs its own `pandoc`+`weasyprint` dependencies via `brew`/`apt`/`dnf`/`pacman` on first run. If the script itself is missing, HALT and name the missing path — do not fall back to calling a converter directly.
...
2. Attach via direct Jira REST API call — see the `atlassian-jira-mcp` skill's "Attaching a file" section for the exact request shape, credential handling, and redaction rule.
